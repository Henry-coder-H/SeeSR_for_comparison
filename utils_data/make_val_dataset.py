...
parser.add_argument("--mode", type=str, default="train", choices=["train", "val"], help="模式：train生成训练数据，val生成验证数据")
parser.add_argument("--num_patches", type=int, default=3000, help="验证模式下生成的patch数量")
parser.add_argument("--val_scale", type=float, default=4.0, help="验证模式下使用的固定scale值")
...
with open(meta_path, "a") as meta_f:
    with torch.no_grad():
        if args.mode == "val":
            # 验证模式：生成固定数量的patches，使用固定的scale值
            print(f"验证模式：生成 {args.num_patches} 个 {args.gt_patch}x{args.gt_patch} 的patches")
            print(f"使用固定scale值: {args.val_scale}")
            patches_generated = 0
            
            # 验证模式使用固定的scale值
            sf_any = args.val_scale
            log_scale = math.log(sf_any)
            
            while patches_generated < args.num_patches:
                for batch_idx, batch in enumerate(tqdm(loader, desc=f"生成验证patches {patches_generated+1}/{args.num_patches}")):
                    if patches_generated >= args.num_patches:
                        break
